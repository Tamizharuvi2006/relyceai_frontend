...
export default function useChat(props) {
  const { currentSessionId, userId, onMessagesUpdate, chatMode: externalChatMode, onChatModeChange: externalOnChatModeChange } = props;
  const { userProfile } = useAuth();

  const core = useChatCore({ currentSessionId, userId, externalChatMode, externalOnChatModeChange, userProfile });
