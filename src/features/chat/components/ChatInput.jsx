...
import { uploadUserFileToBackendOnly } from '../../files/services/fileService.js';
import { useAuth } from '../../../context/AuthContext.jsx';

export default function ChatInput({ onSend, onFileUpload, onFileUploadComplete, botTyping, onStop }) {
    // Enforce dark theme
    const theme = 'dark';
    ...
    const handleFileChange = async e => {
        ...
        const filePreview = { id: Date.now().toString(), name: file.name, size: file.size, type: file.type, file: file };
        ...
        if (onFileUpload) {
            const id = onFileUpload(file.name);
            try {
                if (!user) {
                    console.error('❌ No user found, using guest upload');
                    const resp = await uploadFile(file, null);
                    if (onFileUploadComplete) {
                        onFileUploadComplete(id, !(resp?.error), resp?.url || '');
                    }
                    return;
                }
                const uniqueUserId = userProfile?.uniqueUserId || user.uid;
                const resp = await uploadUserFileToBackendOnly(uniqueUserId, file);
                if (onFileUploadComplete) {
                    const backendFilePath = resp.fileMetadata?.backendPath || resp.backendResponse?.file_saved_to || resp.filePath || '';
                    const fileNameMatch = backendFilePath.match(/-([^-]+\.\w+)$/);
                    const displayFileName = fileNameMatch ? fileNameMatch[1] : file.name;
                    onFileUploadComplete(id, true, displayFileName);
                }
            } catch (error) {
                ...
                try {
                    ...
                    if (userId) {
                        const fallbackResp = await uploadFile(file, userId);
                        if (onFileUploadComplete) {
                            const filePath = fallbackResp?.filePath || fallbackResp?.file_saved_to || file.name;
                            onFileUploadComplete(id, !(fallbackResp?.error), filePath);
                        }
                    }
                } catch (fallbackError) {
            ...
            <div className="relative w-full max-w-[1200px] mx-auto backdrop-blur-xl rounded-2xl p-2.5 shadow-lg transition-all duration-300 chat-input-container bg-zinc-900 border border-emerald-500/30">
                ...
                <div className="flex justify-between items-center w-full mt-2">
                    <div className="flex items-center gap-1">
                        ...
                        <button onClick={handleWebSearch} className={`websearch-button flex items-center gap-1 p-2 rounded-full transition-all text-xs ${isWebSearchActive ? 'bg-[#003925] text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700/60'}`} title={isWebSearchActive ? "Web search enabled - Click to disable" : "Enable web search"}>
                            <Search size={16} className={isWebSearchActive ? "text-white" : ""} />
                            <span className="websearch-text font-medium">websearch {isWebSearchActive && '✓'}</span>
                        </button>
