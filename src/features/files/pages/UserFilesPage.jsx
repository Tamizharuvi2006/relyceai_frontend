...
import { deleteFile } from '../../../utils/api'; // Import delete function
...
const UserFiles = () => {
  ...
  const handleDownload = async (file) => {
    try {
      // For backend-only files, we need to create a download link
      if (file.backendOnly && file.backendPath) {
        // Extract the filename from the backend path
        const fileName = file.originalName || file.fileName;
        const backendUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';
        const downloadUrl = `${backendUrl}/download/${userProfile.uniqueUserId}/${encodeURIComponent(fileName)}`;
        window.open(downloadUrl, '_blank');
      } else if (file.downloadURL) {
        // For Firebase Storage files, use the download URL
        window.open(file.downloadURL, '_blank');
      } else {
        // Fallback: try to construct a path-based download
        const backendUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';
        const fileName = file.originalName || file.fileName;
        const downloadUrl = `${backendUrl}/download/${userProfile.uniqueUserId}/${encodeURIComponent(fileName)}`;
        window.open(downloadUrl, '_blank');
